...
async function loadMoreGenres () {
    ...
    for (const genre of nextBatch) {
        const data = await fetchByGenre(genre.id, false);
...
onMounted(async () =>{
    try {
        currentTvShows.value = await fetchCurrentReleaseMovie(false);
        allGenres.value = await fetchGenreAsync(false);
